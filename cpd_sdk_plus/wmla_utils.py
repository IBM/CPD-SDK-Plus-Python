...
def submit_training(options,rest_host=None,rest_port=-1,jwt_token=None,path_cli='dlicmd.py'):
    ...
    """
    rest_host = fill_in_default_if_none(rest_host,'HOST')
...
# TODO: TO BE DEPRECATED; USE script_prepare(path,variables) in wml_sdk_utils.py 
...
def download_cli(service='dlim',operation_system='linux',rest_host=None):
    """
    Programmatically download WMLA CLI tool.

    service: either dlim (for deployment) or dlicmd (for training)
    operation_system: windows, linux, or mac; this value does not matter if service=="dlicmd", as dlicmd is not specific to your operation system
    rest_host: host name of your WMLA web console link
    """
    if rest_host is None:
        raise Exception('Error: rest_host needs to be specified as the host name of your WMLA web console link, starting with https://')
    
    if service == 'dlicmd':
        subprocess.run(f'wget {rest_host}/ui/tools/dlicmd.py --no-check-certificate',shell=True)
    elif service == 'dlim':
        if operation_system == 'windows':
            ...
            raise Exception('Error: operation_system is not one of windows, linux, or powerai')

        subprocess.run(f'wget {rest_host}/dlim/v1/download/{filename} --no-check-certificate',shell=True)
    else:
        raise Exception('Error: service is not one of dlim or dlicmd')
