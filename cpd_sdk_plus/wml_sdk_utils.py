...
def get_client(credentials=None,space_id=None):
    ...
    if credentials is None:
        if os.getenv('CPD_USERNAME') is not None and os.getenv('CPD_APIKEY') is not None:
            credentials = {'url':os.getenv('CPD_BASE_URL',os.getenv('RUNTIME_ENV_APSX_URL')),
                           'username':os.environ['CPD_USERNAME'],
                           'apikey':os.environ['CPD_APIKEY'],
...
def upload(path,wml_client,fn_target=None,overwrite=False):
    ...
    if os.path.isdir(path):
        subprocess.run(f'cd {escape_path(dir_parent)}; zip -r {escape_path(path_archive)} {escape_path(dir_base)}',shell=True)
...
def function_store(path_script,wml_client,function_name=None,software_spec='default_py3.8',overwrite=True):
    """
    ...
    subprocess.run(f'gzip -kfc {escape_path(path_script)} > {escape_path(path_archive)}',shell=True)
    
    if overwrite:
        function_delete(wml_client,function_name)
    
    software_spec_id =  wml_client.software_specifications.get_id_by_name(software_spec)
    print(f"Using software spec {software_spec} ({software_spec_id})...")
    function_meta_props = {
...
def script_prepare(path,variables={},skip_lines=1,overwrite=False):
    """
    ...
    assert skip_lines >= 0, "skip_lines should be either 0 or a positive number"
    assert isinstance(skip_lines,int), "skip_lines should be an integer"
    lines_final = lines_file[0:skip_lines] + lines + [''.join(lines_file[skip_lines:])]
    
    if not overwrite:
        path_elements = path.split('.')
        path = '.'.join(path_elements[:-1]) + '_edited.' + path_elements[-1]
...
def escape_path(path):
    l_components = [e if e.replace('-','').replace('_','').isalnum() or e=="." else f'"{e}"' for e in path.split('/') ]
    return '/'.join(l_components)
